...
function renderFolderRows(rows) {
  ...
  rows.forEach((rowData) => {
    ...
    preview.src = `/api/fs/thumbnail?path=${encodeURIComponent(rowData.path || "")}&size=72`;
...
function renderFolderGrid(rows) {
  ...
  rows.forEach((rowData) => {
    ...
    image.loading = "lazy";
    image.src = `/api/fs/thumbnail?path=${encodeURIComponent(rowData.path || "")}&size=360`;
  ...
  try {
    ...
    await pollJob(startResponse.job_id, (job) => {
      ...
      const hasPending = state.results.some(
        (result) => !result.error && !(result.move && result.move.moved),
      );
      moveBatchButton.disabled = !hasPending;
    });
  } catch (error) {
  ...
  try {
    const startResponse = await api("/api/folder/move-job", {
      ...
      body: JSON.stringify({
        ...
        results: state.results.filter((result) => !result.error),
      }),
    });
    await pollJob(startResponse.job_id, (job) => {
