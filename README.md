...
自动监控启动时可以选择三种模式：“忽略已有文件，仅处理新增/修改”、“处理所有未处理文件”、“处理所有文件（包括已处理过的）”。默认使用 watchdog/inotify 事件监控：选择忽略已有文件时不会启动全目录扫描，只会在新增、移动进入或修改图片时触发识别；选择处理所有未处理文件时会在启动时扫描一次并跳过指纹未变化的图片；选择处理所有文件时会在启动这一轮重新识别已有图片，包括历史上处理过且指纹未变化的图片，之后仍切换为事件监控。只有环境缺少 watchdog 时才回退为定时扫描。事件监控和轮询后续检查仍会跳过指纹未变化的图片，不会因为多一个或少一个文件就把所有图片重新识别一遍。自动监控面板中的“本次发现”会累计本次启动后检查到的图片，“最近识别”只在实际完成识别后刷新。

“处理所有未处理文件”和“处理所有文件”模式都会使用流水线处理：扫描线程持续发现并检查文件，识别线程同时从待处理队列里取小批图片识别移动。扫描快时会先积压待识别队列，不会等所有图片都完成标记后才开始处理。
