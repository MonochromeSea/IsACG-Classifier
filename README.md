...
自动监控启动时可以选择“忽略已有文件，仅处理新增/修改”，或“处理所有未处理文件”。默认使用 watchdog/inotify 事件监控：选择忽略已有文件时不会启动全目录扫描，只会在新增、移动进入或修改图片时触发识别；选择处理所有未处理文件时会在启动时扫描一次，之后切换为事件监控。只有环境缺少 watchdog 时才回退为定时扫描。无论哪种模式，都只会识别新增图片或文件指纹已变化的图片，不会因为多一个或少一个文件就把所有图片重新识别一遍。

“处理所有未处理文件”模式会使用流水线处理：扫描线程持续发现并检查文件，识别线程同时从待处理队列里取小批图片识别移动。扫描快时会先积压待识别队列，不会等所有图片都完成标记后才开始处理。
